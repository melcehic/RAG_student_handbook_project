...
# --- Config ---
CHROMA_PATH = "C:\\Users\\Melissa\\Bureau\\Bsc 2\\DL\\DL_Student_HB_project\\chroma_db"
...
question = st.text_input("❓ Ta question :", placeholder="What is the name of the program in data science?")

if question:
    collection = load_chroma()
    st.write(f"📦 Nombre de documents dans ChromaDB : {collection.count()}")

    context = retrieve_context(question, collection)

    st.markdown("### 📄 Contexte trouvé")
    st.write(context)

    # Appliquer les 3 modèles
    st.markdown("### 🤖 Réponses des modèles")
    models = load_models()
